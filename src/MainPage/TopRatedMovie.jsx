...
function TopRatedMovie() {
  ...
  useEffect(() => {
    axios
      .get(
        `https://api.themoviedb.org/3/movie/top_rated?api_key=c8b147e9fe4390650885295607b0a593&page=1`
      )
      .then((response) => {
        setdata(response.data.results);
        console.log(response.data.results?.[0]?.vote_average);
        console.log(
          (response.data.results?.[0]?.release_date + "").split("-")[0]
        );
      })
            ...
            data.map((data1, index) => (
              <SwiperSlide>
                <div
                  ...
                  className="max-w-[400px] hover:grayscale-[70%] transition ease-in-out delay-150 w-[175px] ml-2 cursor-pointer"
                >
