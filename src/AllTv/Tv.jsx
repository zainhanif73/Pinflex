...
function Tv({ input, setInput, search, setSearch, region, year }) {
  ...
  useEffect(() => {
    ...
    if ((search && input != "") || (input != "" && page !== 1)) {
      axios
        .get(
          `https://api.themoviedb.org/3/search/tv?api_key=c8b147e9fe4390650885295607b0a593&include_adult=false&page=${page}&query=${input}&region=${region}&year=${parseInt(
            year
          )}`
        )
        .then((res) => {
  ...
  useEffect(() => {
    ...
    if (!search && input === "") {
      axios
        .get(
          `https://api.themoviedb.org/3/tv/popular?api_key=c8b147e9fe4390650885295607b0a593&include_adult=false&page=${page}`
        )
        .then((data1) => {
